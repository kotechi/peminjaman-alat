<x-layouts::app :title="'Daftar Peminjaman'">
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('peminjaman.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium">
                Tambah Peminjaman
            </a>
        </div>
                        ...
                        @forelse($peminjaman as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('peminjaman.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3">Edit</a>
                                    @if($item->status == 'menunggu')
                                        <form method="POST" action="{{ route('peminjaman.confirm', $item) }}" class="inline mr-2">
                                            @csrf
                                            @method('PUT')
